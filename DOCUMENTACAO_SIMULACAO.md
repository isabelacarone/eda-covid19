...
## 8. Guia de Apresentação em Sala

### Roteiro sugerido (15-20 minutos)

```mermaid
timeline
    title Roteiro de Apresentação — Simulação do Spark
    section 0-3 min
        Contexto : Por que simular?
                 : O que o professor pediu
                 : Estratégia adotada
    section 3-8 min
        Engine : Mostrar a classe Column
               : Demonstrar lazy evaluation
               : Mostrar RDD.map + reduceByKey
    section 8-13 min
        Pipeline : Rodar main_simulado.py ao vivo
                 : Mostrar saídas em data/processado_sim/
                 : Abrir simulacao.ipynb
    section 13-18 min
        Visualizações : Fig 02 - MapReduce em gráfico
                      : Fig 03 - Evolução global
                      : Fig 13 - Correlação socioeconômica
                      : Fig 17 - Window Function por país
    section 18-20 min
        Comparativo : Simulação vs. PySpark real
                    : Perguntas
```
